...
## 최신 상태 (2026-02-24) - 템플릿 카드 아이콘 공식 로고 적용

### 1) Scaffolder 템플릿 카드 아이콘 렌더링 방식 변경

- 파일: `packages/app/src/components/scaffolder/TemplateStackCard.tsx`
- 변경:
  - 기존 Material UI 아이콘 중심 렌더링에서
    템플릿 스택별 로고 이미지 렌더링 방식으로 전환
  - `sesac.io/template-icon` annotation 값을 기준으로 로고 매핑

### 2) 대표 로고 매핑 추가

- 적용 대상 예시:
  - `nodejs`, `java`, `ec2`, `s3`, `eks`
- 추가 매핑:
  - `rds`, `dynamodb`, `elasticache`, `kubernetes`, `argo`, `go`, `ray`
- 로고가 없는 경우 기존 fallback 아이콘으로 표시되도록 유지

### 3) 카드 스타일 보정

- 아이콘 박스에 흰색 배경 + 경계선 적용
- 이미지 아이콘 사이즈를 고정(`22x22`)해 카드 간 시각 일관성 확보
